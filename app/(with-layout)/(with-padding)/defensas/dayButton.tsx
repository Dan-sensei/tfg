...
export default function DayButton({
    ...
    const updateRouter = () => {
        ...
        if(searchParams.get("year") !== year.toString() || searchParams.get("month") !== (month+1).toString()){
    ...
    let background = ""
    if(sameMonth){
        background = selected? "text-nova-link  bg-blue-500" : "text-nova-link bg-[#07090c] hover:bg-nova-light2/50 border-1 lg:border-1 border-white/10";
    }
    else{
        background = selected? "text-slate-600  bg-nova-light/20" : "text-slate-600 bg-[#0c0f14]/70 hover:bg-nova-light/20 border-1 lg:border-1 border-white/10";
    }
