...
export async function GET(request: Request) {
    ...
    try {
        ...
        for (const value of Object.values(filterFunctions)) {
            ...
            if (hasRequiredParams && hasOptionalParams) {
                const params = Object.fromEntries(
                    [...value.requiredParams, ...(value.optionalParams || [])].map((param) => [param, searchParams.get(param)])
                );
        ...
        console.error(error);
        return badResponse("Internal server error");
    }
}
...
function getTagsQuery(tags: string): Prisma.Sql {
    ...
    return Prisma.sql`tags @> ARRAY[${Prisma.join(array_tags.map((tag) => Prisma.sql`${tag}`))}]`;
}
