...
type Props = {
    className?: string;
    departments: DepartmentWithTFGCount[];
};
...
export default function DepartmentsForm({ departments, className }: Props) {
    const { data: session } = useSession();
    const [departmentsList, setDepartmentsList] = useState<DepartmentWithTFGCount[]>(departments);
    ...
    const openDeleteDialog = () => {
        if (departmentsList.length > 1 && selectedDepartment) {
            setIsOpen(true);
