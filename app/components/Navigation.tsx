...
const DefaultLink = ({
    ...
    name,
    isCategories,
}: LinkProps & { isCategories?: Boolean }) => {
    return (
        <Link
            color="foreground"
        ...
        >
            {name}
            {isCategories && (
                <IconChevronDown className=" ml-2 mt-1" size={20} />
            )}
        </Link>
    );
};
const TooltipLink = ({ href, name, categoriesElements }: TooltipLinkProps) => {
    const firstColumn = categoriesElements.slice(
        0,
        categoriesElements.length / 2
    );
                ...
                <div className="grid grid-cols-12 gap-4">
                    <div className="col-span-9">
                            ...
                            <div className="">
                                {firstColumn.map((category, index) => (
                                    <Link
                                        ...
                                        className="hover:text-nova-link transition-colors block font-bold pt-1"
                                    >
                            ...
                            <div className="">
                                {secondColumn.map((category, index) => (
                                    <Link
                                        key={index}
                                        href={`/categoria/${category.id}`}
                                        className="hover:text-nova-link transition-colors font-bold block pt-1"
                                    >
                                        <div>{category.name}</div>
                                    </Link>
                                ))}
                        ...
                        <div className="pl-3 text-gray-400">
                            ...
                            >
                                Ver todos{" "}
                                <IconChevronRight
                                    className="inline text-nova-link"
                                    size={20}
                                />
                            </Link>
                        </div>
                    ...
                    <div className="col-span-3 flex flex-col gap-4">
                        <div className="flex-1">
                            <Button
                                href="#"
                                className="w-full h-full font-bold text-nova-link"
                                color="primary"
                                variant="ghost"
                            >
                                Titulaciones
